...
function Berserker:initDPSAbilities()
    ...
    table.insert(self.DPSAbilities, common.getAA('War Cry of the Braxi')) -- Increase Hit Damage Taken by 13% (30 Hits) (Always hit before Dichotomic)
    table.insert(self.DPSAbilities, common.getBestDisc({'Ecliptic Rage', 'Composite Rage', 'Dichotomic Rage'}))
    table.insert(self.DPSAbilities, common.getBestDisc({'Roiling Rage', 'Frothing Rage', 'Seething Rage', 'Smoldering Rage', 'Bubbling Rage'})) -- Add Melee Proc: Bubbling Rage Strike II, 2H Slash Attack for 254 with 53% Accuracy Mod (2 Strikes)
    table.insert(self.DPSAbilities, common.getAA('Binding Axe')) -- Throwing Attack for 500 with 10000% Accuracy Mod
    table.insert(self.DPSAbilities, common.getBestDisc({'Shared Barbarism', 'Shared Violence', 'Shared Atavism', 'Shared Cruelty'})) -- Increase all weapon skills Damage Bonus by 457
    ...
    --table.insert(self.DPSAbilities, common.getBestDisc({'Conqueror\'s Conjunction', 'Vindicator\'s Coalition', 'Demolisher\'s Alliance'}))
    --table.insert(self.DPSAbilities, common.getBestDisc({'Unthinking Retaliation', 'Instintive Retaliation', 'Reflexive Retaliation'}))
...
function Berserker:initBuffs()
    table.insert(self.combatBuffs, common.getAA('Desperation', {combat=true})) -- Increase Melee Haste v3 by 25% (Just regular haste, already will be max on raid)
    table.insert(self.combatBuffs, common.getAA('Blood Pact', {combat=true})) -- Add Melee Proc: Blood Pact Strike XXII (2750)
