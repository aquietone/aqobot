...
local SETTINGS_FILE = ('%s/mnkbot_%s_%s.lua'):format(mq.configDir, mq.TLO.EverQuest.Server(), mq.TLO.Me.CleanName())
mnk.load_settings = function()
    local settings = config.load_settings(SETTINGS_FILE)
    if not settings or not settings.mnk then return end
    for setting,value in pairs(settings.mnk) do
        OPTS[setting] = value
    end
end

mnk.save_settings = function()
    persistence.store(SETTINGS_FILE, {common=config.get_all(), mnk=OPTS})
end

mnk.setup_events = function()
    -- no-op
end


mnk.process_cmd = function(opt, new_value)
    if new_value then
...
mnk.main_loop = function()
    ...
    if not state.pull_in_progress then
        ...
        if config.MODE:is_assist_mode() then
            assist.check_target(mnk.reset_class_timers)
            assist.attack()
        end
