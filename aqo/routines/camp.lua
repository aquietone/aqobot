...
camp.check_camp = function()
    ...
    if mq.TLO.Zone.ID() ~= camp.ZoneID then
        printf(logger.logLine('Clearing camp due to zoning.'))
        camp.Active = false
        return
    end
...
camp.set_camp = function(reset)
    ...
    if mode:set_camp_mode() then
        ...
        printf(logger.logLine('Camp set to \ayX: %.02f Y: %.02f Z: %.02f R: %s H: %.02f\ax', camp.X, camp.Y, camp.Z, config.CAMPRADIUS, camp.Heading))
